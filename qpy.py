...
make_pseudo_terminal='''
def pseudo_terminal(message: str = "pseudo_terminal() ⟹ Entering interactive session!",modifier=None,**additional_vars):
    exec("ans=None",globals())
    global ans,last_assignable,last_assignable_candidate
    last_assignable=last_assignable_candidate=None
    # @author: Ryan Burgert 2016，2017
    try:import readline# Makes pseudo_terminal nicer to use if in a real terminal (AKA if using pseudo_terminal on the terminal app on a mac); aka you can use the up arrow key to go through history etc.
    except:pass
    from r import namespace
    from r import rinsp# ⟵ This is just plain useful when using the pseudo_terminal; like bread and butter.
    original_globals=globals()# pt overwrites global variables instead of local variables by default. If we call globals().update(original_globals), we will leave the original namespace alone (apart from perhaps adding a few new names).
    overwrite_globals=True
    def restore_original_global_variables(original_globals,exception_names=()):# If we decide we want to
        # exception_names purpose: By default, if overwrite_globals,
        for name in original_globals:
            if name not in exception_names:
                if name in globals():
                    globals().update({name:eval(name)})
                else:
                    del globals()[name]# Delete all newly created variables except for the ones in exception_names
    # TODO: Make better error reports than are available by defauly in python! Let it debug things like nested parenthesis and show where error came from instead of just throwing a tantrum.


    from r import fansi_print,fansi,string_from_clipboard,mini_editor,python_input# Necessary imports for this method to function properly.
    ...
    fansi_print(message,'blue','bold')
    ans=None
    def set_ans(val):
        global ans
        fansi_print("ans = " + str(val),'green')
        ans=val
        globals().update({'ans':val})
        locals().update({'ans':val})
    successful_command_history=[]
    ans_history=[ans]
    assignable_history={}
    assignable_history_redos={}
    dir_before_attempt=set()
    result=None# To prevent them from appearing in VARS. If you add more things to pseudo_terminal it might be nessecary to add even more things here.
    header=" ⮤ "
    # TODO original_namespace=dir()# Experimental. Used for a "CLEAR" command to erase all variables created while in pseudo_terminal
    # full_exception_with_traceback=user_message=error_message_that_caused_exception=x=result=dir_before_attempt=None
    user_created_var_names=set()

    allow_keyboard_interrupt_return=False

    def delete_vars(variable_names):# For the user of the pseudo_terminal to call manually after calling 'VARS' if they so feel like it.
        for n in variable_names:
            print("del "+n)

    def globalize_vars(variable_names):# For the user of the pseudo_terminal to call manually after calling 'VARS' if they so feel like it.
        for n in variable_names:
            fansi_print("globals().update({'"+n+"':"+n+"})",'magenta')
    ...
    try:
        while True:
            ...
            try:
                for thinggy in assignable_history.copy():
                    try:
                        exec(thinggy+"=assignable_history["+repr(thinggy)+"][-1]",globals())
                    except:
                        # print("KILLED "+thinggy)
                        del assignable_history[thinggy]
                for user_created_var_name in set(user_created_var_names)|set(assignable_history):
                    try:
                        eval1 = eval(user_created_var_name,globals())
                    except Exception as asijdasd:
                        print(asijdasd)
                        # print("IM GONNA SET MYSELF ON FIRE!")
                        if user_created_var_name in assignable_history:
                            if user_created_var_name in assignable_history:
                                # print("chrome dome")
                                del assignable_history[user_created_var_name]
                            if user_created_var_name in user_created_var_names:
                                # print("redrum")
                                del user_created_var_names[user_created_var_name]
                    if not user_created_var_name in assignable_history or not assignable_history[user_created_var_name]:
                        assignable_history[user_created_var_name]=[eval1]
                        # print("kachink "+user_created_var_name)
                    else:
                        try:
                            if assignable_history[user_created_var_name][-1]!= eval1:# BROKE ON NUMPY ARRAYS 'truth value is ambiguous blah blah' could use 'is not' instead of != but that might eat memory for no reason
                                assignable_history[user_created_var_name].append(eval1)
                        except:
                            if assignable_history[user_created_var_name][-1]is not eval1:# could use 'is not' instead of != but that might eat memory for no reason
                                assignable_history[user_created_var_name].append(eval1)
                            # print("kachink "+user_created_var_name)
            except BaseException as asijdasd:
                from r import print_stack_trace
                fansi_print("OOP...borked...",'red','bold')
                print_stack_trace(asijdasd)
            try:
                try:
                    # user_message=(input(fansi(" ⮤ ",'cyan','bold')))
                    user_message=(python_input(globals()))
                    allow_keyboard_interrupt_return=False
                    ...
                    if allow_keyboard_interrupt_return:
                        ...
                        raise
                user_created_var_names=user_created_var_names.intersection(eval(namespace))# Make sure that the only variables in this list actually exist. For example, if we use 'del' in pseudo_terminal, ∄ code to remove it from this list (apart from this line of course)
                try:
                    if user_message == 'RETURN':
                        if not user_created_var_names.issubset(set(list(globals()))):
                            fansi_print("r.pseudo_terminal() ⟹ Warning: ［u ⊈ g， where u﹦user_created_var_names ⋀ g﹦global_var_names］",'blue','bold')
                            fansi_print("You have created new variables inside pseudo_terminal. If you return now, these variables will be lost (because they are likely all local variables).",'blue')
                            # if(user_created_var_names.)
                            fansi_print("The variable names are shown below:",'blue','bold')
                            fansi_print(str(user_created_var_names),'blue')
                            fansi_print("If you would like to return anyway, type 'RETURN NOW'",'blue','bold')
                        else:
                            if ans is None:
                                fansi_print("r.pseudo_terminal() ⟹ Session end. No value returned.",'blue','bold')
                            else:
                                fansi_print("r.pseudo_terminal() ⟹ Session end. Returning ans = " + str(ans),'blue','bold')
                            return ans
                    elif user_message=='RETURN NOW':
                        if ans is None:
                            fansi_print("r.pseudo_terminal() ⟹ Session end. No value returned.",'blue','bold')
                        else:
                            fansi_print("r.pseudo_terminal() ⟹ Session end. Returning ans = " + str(ans),'blue','bold')
        ...
        "+ 8" is parsed to "ans + 8" and ".shape" is parsed into "ans.shape"
        "a+=" is parsed to "a += ans" and "a=" is parsed into "a=ans"
        "+=" is parsed to "ans += ans"
        "=8" is parsed to "◊ = 8"
        "+=8" is parsed to "◊ += 8"
        shell_command killall -u Ryan Python ⟶ Parsed command into 'shell_command( ' killall -u Ryan Python')'
        a 3 ⟶ a[3] if a is list
        a 3 : ⟶ a[3,:]
        WITH NEW TERMINAL
        command L to erase things like 'MORE'; shift left/shift right
        (,[,{ and ) etc autocompletion
        "Hello"?    "Hello"??   "Hello"???
        ?   ??    ???    ????  ⟶ rinsp ans (different ↈ ? for options)
        Enter 'MODIFIER ON', 'MODIFIER OFF', 'VARS', 'MORE', 'RETURN NOW', 'EDIT', 'GHISTORY', 'COPY', 'SPASTE', 'CHISTORY', 'DITTO', 'HIST', 'UNDO ◊', 'HIST ◊' ""","blue")
                        ...
                        for x980 in successful_command_history:
                            fansi_print(x980 if '\\n' not in x980 else '','green')  # x if '\\n' not in x else '' ≣ '\\n' not in x and x or ''
                        ...
                        fansi_print("The last error-causing command is shown below in magenta:",'red','bold')
                        fansi_print(error_message_that_caused_exception,'magenta')
                        fansi_print(full_exception_with_traceback if full_exception_with_traceback is not None else "   (The last command did not cause an error)",'red')# full_exception_with_traceback is None ⟹ Last command did not cause an error
                        ...
                        set_ans(user_created_var_names)
                    elif user_message == "?":
                        ...
                        rinsp(eval(user_message[:-5],globals()),1,1,1)
                    elif user_message.endswith("????"):
                        fansi_print("◊???? ⟹ rinsp(◊,1,0,1)","blue")
                        rinsp(eval(user_message[:-4],globals()),1,0,1)
                    elif user_message.endswith("???"):
                        fansi_print("◊??? ⟹ rinsp(◊,1,1)","blue")
                        rinsp(eval(user_message[:-3],globals()),1,1)
                    elif user_message.endswith("??"):
                        fansi_print("◊?? ⟹ rinsp(◊,1)","blue")
                        rinsp(eval(user_message[:-2],globals()),1)
                    elif user_message.endswith("?"):
                        fansi_print("◊? ⟹ rinsp(◊)","blue")
                        rinsp(eval(user_message[:-1],globals()))
                    elif user_message in {"#UNDO","UNDO"}:
                        fansi_print("UNDO ⟹  ans = ‹the previous value of ans›:","blue")
                        if len(ans_history)<2:
                            fansi_print("\t[Cannot UNDO ans right now because ans_history only has one element]",'red')
                        else:
                            ans_history.pop()
                            set_ans(ans_history[-1])
                    ...
                    elif user_message.startswith("UNDO ") or user_message.startswith("#UNDO "):# assignable UNDO
                        assignable=user_message[6 if user_message[0]=='#' else 5:].lstrip().rstrip()
                        if len(assignable_history[assignable])<2:
                            fansi_print("\t[Cannot UNDO "+assignable+" right now because it's history in assignable_history only has one element. ]",'red')
                        else:
                            undo_assignable_command=assignable+"=assignable_history["+repr(assignable)+"][-1]"
                            if assignable in assignable_history_redos:
                                assignable_history_redos[assignable].append(assignable_history[assignable].pop())
                            else:
                                assignable_history_redos[assignable]=[assignable_history[assignable].pop()]
                            exec(undo_assignable_command,globals())
                            eval2 = eval(assignable,globals())
                            valdisp=str(eval2)
                            if len(valdisp)>2000:# Don't make spam
                                valdisp="‹the previous value of "+assignable+"›"
                            fansi_print("UNDO ⟹  "+assignable+" = "+valdisp,'blue')
                            successful_command_history.append(("#"*(user_message[0]!="#")) +user_message)
                    elif user_message==("HIST"):# assignable UNDO
                        from r import display_dict
                        display_dict({k:len(assignable_history[k])for k in assignable_history})
                    elif user_message.startswith("HIST "):# assignable UNDO
                        assignable=user_message[5:].lstrip().rstrip()
                        from r import  display_list
                        display_list(assignable_history[assignable])
                    else:
                        ...
                        if user_message == 'DITTO':
                            if not successful_command_history:
                                ...
                                fansi_print(user_message,"yellow")
                        if 'EDIT' in user_message:
                            from r import namespace
                            user_message=mini_editor("",eval(namespace))
                        ...
                        if len(user_message.split("\\n"))==1:# If we only have 1 line: no pasting
                            ...
                            if len(_thing)>1:
                                ...
                                if all(map(is_literal,_thing)):# If there are no ';' or ',' in the arguments; just 'rinsp' or 'ans' etc
                                    ...
                                    flaggy=True
                                if flaggy:
                                    fansi_print("Parsed command into '"+user_message+"'",'magenta')
                        if user_message.lstrip():
                            try:
                                ...
                                if last_assignable and (arg_0[0]=='=' and arg_0[1]!="=" or arg_0[0:2] in ['+=','-=','*=','/=','&=','|=','^=','%='] or arg_0[:3] in ['//=','**=','<<=','>>=']):
                                    if not last_assignable in assignable_history:
                                        assignable_history[last_assignable]=[]
                                    else:
                                        assignable_history[last_assignable].append(eval(last_assignable,globals()))
...
#⁠⁠⁠⁠                                                                                                             ⎩                                                                                                                                                                                                                                               ⎭
                            ...
                            # print("TRUCK")
                            # print(last_assignable_candidate)
                        else:
                            # last_assignable_candidate=None
                            pass
                        #endregion

                        while True:  # Try to correct any errors we might find in their code that may be caused mistakes made in the pseudo_terminal environment
                            try:
                                dir_before_attempt=eval(namespace)



                                result=eval(user_message,globals())
                                user_created_var_names|=eval(namespace,globals()) - dir_before_attempt
                                # if overwrite_globals:# We take all the user_created_var_names and make them global. This might cause problems in the future, I don't know! This to prevent things like "a=3;b=lambda:print(a);b()" from causing errors, which was a significant problem.
                                #     for n in user_created_var_names:
                                #         try:
                                #             globals().update({n:eval(n)})
                                #         except:
                                #             print("This should not cause an error eval")
                                ...
                                try:
                                    dir_before_attempt=eval(namespace)
                                    exec(user_message,globals())
                                    user_created_var_names|=eval(namespace,globals()) - dir_before_attempt
                                    # if overwrite_globals:  # We take all the user_created_var_names and make them global. This might cause problems in the future, I don't know! This to prevent things like "a=3;b=lambda:print(a);b()" from causing errors, which was a significant problem.
                                    #     for nnn9 in user_created_var_names:
                                    #         try:
                                    #             globals().update({nnn9:eval(nnn9)})
                                    #         except:
                                    #             print("This should not cause an error exec")
                                    break
                                except IndentationError as asdjopoasd:
                                    if header in user_message:  # They probably just copied and pasted one of their previous commands from the console. If they did that it would contain the header which would cause an error. So, we delete the header.
                                        print(type(asdjopoasd))
                                        fansi_print("That command caused an error, but it contained '" + header + "' without quotes. Running your command without any '" + header + "'_s, shown below in magenta:","red","bold")
                                        user_message=user_message.replace(header,"")  # If we get an error here, try getting rid of the headers and then try again via continue...
                                        fansi_print(user_message,"magenta")
                                        # continue # ⟵ I believe this might be redundant...
                        ...
                        if user_message != "":
                            ...
                            if last_assignable_candidate:
                                ...
                                import r_iterm_comm
                                r_iterm_comm.last_assignable_comm=last_assignable
                            if last_assignable:
                                try:
                                    eval3 = eval(last_assignable,globals())
                                    if last_assignable in assignable_history:
                                        try:
                                            if assignable_history[last_assignable]!=eval3:# broke on numpy arays
                                                assignable_history[last_assignable].append(eval3)
                                        except:
                                            if assignable_history[last_assignable]is not eval3:
                                                assignable_history[last_assignable].append(eval3)
                                    else:
                                        assignable_history[last_assignable]=[eval3]
                                except:
                                    pass
                                # last_assignable=None
                            # full_exception_with_traceback=error_message_that_caused_exception=None# There were no errors in this command, so to avoid confusion remove the last exception that we may have had ⟵ Removed this line because I found it restrictive to my freedom in practice
                except Exception as asdjopoasd:
        ...
        NOTE: This will be the last time you see this message, unless you enter 'HELP' without quotes.""",'red','bold')
                    ...
                    full_exception_with_traceback=''.join(format_exception(asdjopoasd.__class__,asdjopoasd,asdjopoasd.__traceback__))
                    error_message_that_caused_exception=user_message# so we can print it in magenta if asked to by 'MORE'
                    print(fansi('ERROR: ','red','bold')+fansi(''.join(format_exception_only(asdjopoasd.__class__,asdjopoasd))[:-1],'red'))  # Display the error traceback SET:HIDELEVEL1:TRUE
            except KeyboardInterrupt:
                ...
                print(fansi("ERROR: Caught some error, idk what it was but it wasn't an Exception",'red','bold'))
                from r import print_stack_trace
                print_stack_trace(bee)
    except BaseException as asijdasd:
        from r import print_stack_trace
        print(fansi('FATAL ERROR: Something went very, very wrong. Printing a stack trace + HISTORY so you can recover!','red','bold'))
        print_stack_trace(asijdasd)
        print_history()
...
def text_to_speech(text: str,voice: str = None,run_as_thread=True):
    ...
    kwargs=dict(text=text,run_as_thread=run_as_thread)
...
from r_credentials import Đ_gmail_address   # ⟵ The email address we will send emails from and whose inbox we will check in the methods below.
from r_credentials import Đ_gmail_password  # ⟵ Please don't be an asshole: Don't steal this account! This is meant for free use!
...
def OSC_output(address,value):
    ...
    from TestOSC import SimpleUDPClient
    global _OSC_client
    if not _OSC_client:
...
def shell_command(command: str,return_printed_stuff_as_string: bool = True) -> str or None:
...
def print_stack_trace(error: Exception,full_traceback: bool = True):
    from traceback import format_exception,format_exception_only
    # ⁠⁠⁠⁠    ⎧                                                                                                                                                                                                                          ⎫
    # ⁠⁠⁠⁠    ⎪                                                            ⎧                                                                                                                                                            ⎫⎪
    # ⁠⁠⁠⁠    ⎪                                                            ⎪       ⎧                                                           ⎫                               ⎧                                            ⎫           ⎪⎪
    # ⁠⁠⁠⁠    ⎪     ⎧                                             ⎫        ⎪       ⎪                ⎧                                         ⎫⎪                               ⎪                     ⎧                     ⎫⎪⎧   ⎫      ⎪⎪
    print(fansi('r.print_stack_trace: ERROR: \n','red','bold') + fansi(''.join(format_exception(error.__class__,error,error.__traceback__)) if full_traceback else ''.join(format_exception_only(error.__class__,error))[:-1],'red'))
    # ⁠⁠⁠⁠    ⎪     ⎩                                             ⎭        ⎪       ⎪                ⎩                                         ⎭⎪                               ⎪                     ⎩                     ⎭⎪⎩   ⎭      ⎪⎪
    # ⁠⁠⁠⁠    ⎪                                                            ⎪       ⎩                                                           ⎭                               ⎩                                            ⎭           ⎪⎪
    # ⁠⁠⁠⁠    ⎪                                                            ⎩                                                                                                                                                            ⎭⎪
    # ⁠⁠⁠⁠    ⎩                                                                                                                                                                                                                          ⎭
...
from ptpython.prompt_style import PromptStyle,ClassicPrompt
import ptpython.prompt_style as ps
...
def edit(file_or_object):
    if isinstance(file_or_object,str):
        return shell_command("atom "+repr(file_or_object))# Idk if there's anything worth returning but maybe there is?
    else:
        import inspect
        return edit(inspect.getfile(inspect.getmodule(file_or_object)))
...
if __name__ == "__main__":
    exec(make_pseudo_terminal)
    # noinspection PyUnresolvedReferences
    pseudo_terminal("Ryan Burgert's \"r\" Module: pseudo_terminal() ⟹ Entering interactive session!")
# endregion
